...
export const useQuestionsStore = defineStore('questionsStore', {
  ...
  actions: {
    ...
    async getLastShortAnswer() {
      ...
      try {
        let response = await axios.get(
          `https://fuberlin.nvii-dev.com/wp-json/wp/v2/antworten_kurzfrageb?author=${userId}&per_page=1`
        );

        // JSON responses are automatically parsed.
        console.log('questionsStore - getLastShortAnswer - response', response);
        console.log(
          'questionsStore - getLastShortAnswer - response - result',
          response.result
        );
    ...
    async countShortAnswers() {
      ...
      try {
        let response = await axios.get(
          `https://fuberlin.nvii-dev.com/wp-json/wp/v2/antworten_kurzfrageb?author=${userId}&per_page=100`
        );

        // JSON responses are automatically parsed.
        console.log('questionsStore - countShortAnswers - response', response);
        console.log(
          'questionsStore - countShortAnswers - response - result',
          response.result
        );

        if (response) {
          this.totalShortAnswers = response.data.length;
        }
